...
# SNAME=$(aws ssm get-parameter --name "/data-syn-poc-gpu/group-1/service-name" | jq --raw-output .Parameter | jq -r ."Value")
...
# | jq '.' --arg NDI $DOCKER_IMAGE ".taskDefinition.containerDefinitions[0].image=$NDI")
...
REGISTERED_TASK=$(aws ecs register-task-definition xw\
--family $TASK_FAMILY  \
--requires-compatibilities EC2 \
--task-role-arn "arn:aws:iam::45xxxxxx:role/data-syn-poc-gpu-group-1-task-execution-role" \
--execution-role-arn "arn:aws:iam::4566xxxxxx148:role/dxwata-syn-poc-gpu-group-1-task-execution-role" \
--memory 102400 \
--cpu 10240 \
--cli-input-json "$(echo $FINAL_TASK)")

# --memory 200356 \ --network-mode awsvpc \
...
if [ -n "$NEW_REVISION" ]; then
    ...
    # --network-configuration "awsvpcConfiguration={subnets=[subnet-03631bfxxxx],securityGroups=[sg-0652xxxx2a],assignPublicIp=DISABLED}"
